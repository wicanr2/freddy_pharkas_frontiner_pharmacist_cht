...
**1993 年那張廣告上的遊戲，現在整套是中文的。**
Sierra 的西部喜劇冒險《Freddy Pharkas: Frontier Pharmacist》，台灣當年以《多情藥師酷牛仔》
的譯名上市卻始終沒有中文版。這個專案把它補上——不改遊戲一個位元組，中文直接畫在 ScummVM 的引擎裡。

![同一句話，英文原版與繁體中文化的對照](images/compare_f8.png)

- **5,166 / 5,181 則文字**已翻譯（99.7%），對白、旁白、道具欄、選單、藥品名、片頭片尾、結局都在內
...
廣告上那四格遊戲畫面，其中一格是鎮上的酒吧。三十年後同一個房間長這樣：

![廣告上的酒吧，與今天的繁體中文版](images/compare_1993.png)

## 這個中文化包含什麼

翻的不只是玩家一路點過去會看到的對白。下面四張是實機畫面：
選單、配藥檯、街景、店內，各自代表一條不同的繪字路徑。

| | |
|---|---|
| ![標題選單](images/shot_menu_cht.png) | ![配藥檯](images/shot_counter.png) |
| 標題選單：載入、序幕、開始、說明、離開——不只對白，連 UI 都翻了 | 配藥檯。這款遊戲真正的解謎核心：照手冊的配方秤重、加熱、攪拌 |
...
- **不只主線對白**：旁白、道具欄品名、主選單與控制面板、藥品與化學品名都翻了。
- **連片尾都翻**：片頭片尾字幕、九種結局畫面、演職員表。演職員表的真人姓名維持拉丁字母。
...
- 背景美術裡畫死的英文招牌（`MOM'S CAFE`、`Prescriptions`）沒有改。
- 上緣 icon bar 的 `SCORE` 是畫在美術裡的英文字，同樣保留。
