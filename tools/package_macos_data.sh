...
# 注入多情藥師酷牛仔 繁中資料(dist-cht/ 的 translation.tsv + freddy_big5.fnt + freddy_big5_hi.fnt)+ README,
# 重新打包成可交付檔。在 CI runner 內跑(bash 內建即可,不需 docker/python)。
#
# 本作只有單一 SCI1.1 VGA 版本,不分 edition,故不帶 edition 參數。
#
...
[ -f "$ROOT/dist-cht/freddy_title.ovl" ] && cp "$ROOT/dist-cht/freddy_title.ovl" "$CHT_DIR/"   # 中文標題疊圖
echo ">>    staged $(ls "$CHT_DIR" | wc -l) 個中文資料檔 → $CHT_DIR"

README="$APP/Contents/Resources/README-cht.txt"
cat > "$README" <<'EOF'
多情藥師酷牛仔（Freddy Pharkas: Frontier Pharmacist）繁體中文化 — macOS 版

本包內容
--------
- patched ScummVM 執行檔（含 Big5 繪字、ZH_TWN 語言支援、hi-res 640x400 對白文字的引擎改動）
- cht-data/：中文資料（translation.tsv 對白/訊息、freddy_big5.fnt 低解析字型、freddy_big5_hi.fnt hi-res 字型、freddy_title.ovl 中文標題疊圖）
- 本說明檔

本包【不含】原遊戲資源，也不含 Roland MT-32 ROM（版權因素不隨包分發）。
請自備合法取得的多情藥師酷牛仔（SCI/DOS 版）遊戲檔。
...
repo：https://github.com/wicanr2/freddy_pharkas_frontiner_pharmacist_cht.git
EOF
...
mkdir -p "$OUT"
LABEL="FreddyPharkas-CHT-macos-universal"
