...
NAME_FIXES = [
    ...
    ("普林姆小姐", "潘妮洛普小姐"),
    ("普林姆", "潘妮洛普"),
    # 譯文覆核抓到的跨批次地名/人名分歧(街名同一條卻三種寫法最容易讓玩家覺得走錯地方)
    ("平衡街", "貝倫斯街"),           # Balance Street,呼應角色 P. H. Balance
    ("教育街", "教鬱遜街"),           # Edukashun Street:原文刻意拼錯,譯名保留拼錯感
    ("教育巷", "教鬱遜巷"),
    ("菲尼亞斯", "菲尼斯"),           # Phineas
    ("佛雷德里克", "佛萊德瑞克"),     # Frederick
    ("潘妮洛普·潘妮洛普", "潘妮洛普"),  # Penelope Primm 姓氏被誤植成又一次名字
    # 021–029 覆核:同一樣東西在道具欄與台詞裡叫不同名字,玩家會對不起來
    ("切克姆", "切肯"),               # Sheriff Checkum P. Shift
    ("保管箱", "保險箱"),             # safe-deposit box
    ("領取憑證", "取件單"),           # Claim Check(UI 用短的)
    ("純溶液", "淨水劑"),             # Pure Solution:台詞一律叫淨水劑
    ("冰鑿", "冰錐"),                 # Ice Pick
    ("帕夫", "小噗"), ("史波特", "小點"),  # 童書角色 Puff and Spot
    ("哈伯雷", "拉伯雷"),             # Rabelais 通行譯名
    ("麥格魯夫", "麥格夫"),           # McGruff
]
